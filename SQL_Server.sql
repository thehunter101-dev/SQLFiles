...
INSERT INTO PRODUCTO
    (IDCATEGORIA, NOMBRE, STOCK, ACTIVO)
VALUES
...
INSERT INTO COMPRA
    (IDPROVEEDOR, FECHA_REGISTRO, SUBTOTAL, IVA, DSCTO, TOTAL, PAGADO)
VALUES
...
INSERT INTO COMPRADETALLE
    (IDCOMPRA, IDPRODUCTO, CANT, PRECIO, SUBTOTAL)
VALUES
...
---Funcion que retorna la cantidad de productos por categoria
GO

CREATE FUNCTION F_TOTAL_PRODUCTO
(
...
BEGIN
    DECLARE @TOTAL INT
    SELECT @TOTAL= COUNT(P.IDPRODUCTO)
    FROM CATEGORIA C
...
GO
DECLARE @TOTAL INT
EXEC @TOTAL = F_TOTAL_PRODUCTO 'Laptops'
PRINT @TOTAL

GO

--- Total de compras pagadas
CREATE FUNCTION F_TOTAL_COMPRAS_PAGADAS
(
    @F VARCHAR(20) = NULL,
    @M VARCHAR(20) = NULL
)
...
BEGIN
    DECLARE @TOTAL INT
    DECLARE @FECHA VARCHAR(7)
    IF @F IS NULL OR @M IS NULL
    BEGIN
        SET @FECHA = FORMAT(GETDATE(),'yyyy-MM')
    END
    ELSE
    BEGIN
        SET  @FECHA = CONCAT(@F,'-',@M)
    END
    SELECT @TOTAL = COUNT(*)
    FROM COMPRA
    WHERE PAGADO = 1 AND CONVERT(VARCHAR(7), FECHA_REGISTRO,120) = @FECHA;
    RETURN @TOTAL
END

GO
SELECT dbo.F_TOTAL_COMPRAS_PAGADAS('2025','08') AS COMPRASPAGADAS
...
INSERT INTO dbo.COMPRA
    (IDPROVEEDOR, FECHA_REGISTRO, SUBTOTAL, IVA, DSCTO, TOTAL, PAGADO)
VALUES
    (1, CONVERT(DATE,GETDATE()), 2000, 40.00, 15, 2500, 0)
GO
---CREAR UNA FUNCION PARA RETONAR EL TOTAL DE DESCUENTOS.
CREATE FUNCTION F_TOTAL_DESCUENTOS_PROVEDOR
(
    @RUC VARCHAR(13)
)
RETURNS MONEY
...
GO
SELECT dbo.F_TOTAL_DESCUENTOS_PROVEDOR('1790012345001') AS DESCUENTOS
GO
----Crear una función para retornar el total obtenido de una categoría
CREATE FUNCTION F_FACTURAS_PAGADAS_CATEGORIA
(
    @CA VARCHAR(100)
)
RETURNS REAL
...
BEGIN
    DECLARE @TOTAL REAL
    SELECT @TOTAL = SUM(CO.TOTAL)
    FROM dbo.COMPRA CO
        INNER JOIN dbo.COMPRADETALLE COD
        ON COD.IDCOMPRA = CO.IDCOMPRA
        INNER JOIN dbo.PRODUCTO P
        ON COD.IDPRODUCTO = P.IDPRODUCTO
        INNER JOIN dbo.CATEGORIA C
        ON P.IDCATEGORIA = C.IDCATEGORIA
    WHERE CO.PAGADO = 1 AND C.NOMBRE = @CA
    RETURN @TOTAL
END
GO
SELECT dbo.F_FACTURAS_PAGADAS_CATEGORIA('Teclados') AS 'CATEGORIA PAGADA'